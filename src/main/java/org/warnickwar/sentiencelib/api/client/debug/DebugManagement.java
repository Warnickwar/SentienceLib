...
public final class DebugManagement {
    ...
    public static void update(float delta) {
        ...
        while (!cachedListToClear.isEmpty()) {
            removeInformation(cachedListToClear.removeFirst());
        }
    }
    ...
    public static void render(PoseStack poseStack, MultiBufferSource.BufferSource buffer, double camX, double camY, double camZ) {
        Map<DebugSystem, Collection<DebugInformation>> queued = getQueuedRenderInformation();
        queued.forEach((system, informationList) -> {
            // Setup system for preliminary rendering
            system.preRender(informationList);

            informationList.forEach(info -> system.render(minecraftInstance, info, poseStack, buffer, camX, camY, camZ));

            // Clean up system after rendering
            system.postRender(informationList);
        });
    }
    ...
    public static void handleNewInformation(S2CDebugInformationPacket packet) {
        ...
        createdArchetypes.values().forEach(a -> {
            if (a.isInArchetype(info)) {
                a.add(info.getUUID(), entry);
            }
        });
    }
    ...
    public static boolean toggleSystem(ResourceLocation renderId) {
    ...
    static void addNewSystem(@NotNull ResourceLocation id, @NotNull DebugSystem system, boolean defaultState) {
