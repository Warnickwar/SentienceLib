package org.warnickwar.sentiencelib.core.planning;
...
import org.warnickwar.sentiencelib.core.actions.ActionPlan;
...
public final class Planners {
    ...
    @Nullable
    public static ActionPlan DFSPlan(Set<IdentifiedData<Desire>> desires,
                                     Set<IdentifiedData<Action>> actions,
    ...
    private static boolean findPathDFS(Node parent, Set<IdentifiedData<Action>> actions, Map<IdentifiedData<Belief>, Boolean> evals) {
        ...
        // No need to rerun this for every action, the previous iteration is already true
        HashSet<IdentifiedData<Belief>> originalRequired = new HashSet<>(parent.requiredEffects);
        originalRequired.removeIf(belief -> getBeliefResult(belief, evals));
    ...
    private static boolean getBeliefResult(IdentifiedData<Belief> belief, Map<IdentifiedData<Belief>, Boolean> evaluations) {
        ...
        if (!evaluations.containsKey(belief)) {
            // Cache so we don't deal with evaluating the value AGAIN, as it can be potentially costly and not thread-safe.
            // Ideally, all Beliefs should be cached before planning.
    ...
    private static class Node {
        ...
        private final Set<IdentifiedData<Belief>> requiredEffects;
        private final List<Node> leaves;

        private final double appeal;

        public Node(@Nullable Node parent, @Nullable IdentifiedData<Action> action, Set<IdentifiedData<Belief>> requiredEffects, double appeal) {
            this.parent = parent;
            this.action = action;
